...
(function () {
  ...
  class DetailedIsoFileAtComponent extends pulseComponent.PulseParamAutoPathSingleRequestComponent {
    ...
    initialize () {
      ...
      // Title
      let title = this.getTranslation('detailsViewSubTitles.isofile', 'NC program');
      let spanTitle = $('<span></span>').addClass('detailedisofileat-title-span')
        .html(title);
    ...
    validateParameters () {
      if ((!this.element.hasAttribute('machine-id'))
        ...
        console.error('missing attribute machine-id in detailedisofileat.element');
        // Delayed display :
      ...
      if (!this.element.hasAttribute('when')) {
        console.error('missing attribute when in detailedisofileat.element');
        // Delayed display :
    ...
    getShortUrl () {
      ...
      return 'IsoFileSlots?MachineId='
        + this.element.getAttribute('machine-id')
        + '&Range='
        + pulseUtility.createSingleRangeForWebService(this.element.getAttribute('when'));
    }

    refresh (data) {
      $(this._detailedContent).empty();

      if (0 < data.IsoFileSlots.length) {
        let divRange = $('<div></div>').addClass('detailed-range');
        let divDetails = $('<div></div>').addClass('detailed-data');

        // RANGE
        let tmpRange = pulseRange.createDateRangeFromString(data.IsoFileSlots[0].Range);
        let rangeDisplay = pulseUtility.displayDateRange(tmpRange, true);
        let spanRange = $('<span></span>').addClass('detailedisofileat-range-span')
          .html(rangeDisplay);
        $(divRange).append(spanRange);

        // DETAILS
        let display = data.IsoFileSlots[0].Display;
  ...
  pulseComponent.registerElement('x-detailedisofileat', DetailedIsoFileAtComponent, ['machine-id', 'when', 'datetime-context', 'machine-context']);
})();
