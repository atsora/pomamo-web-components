...
/**
 * Build a custom tag <x-isofileslotbar> to display an isofileslotbar bar component. This tag gets following attribute : 
 *  machine-id : Integer
 *  height : Integer
...
(function () {
  ...
  class IsoFileSlotBarComponent extends pulseComponent.PulseParamAutoPathRefreshingComponent {
    ...
    */
     ...
     */
    getShortUrl () {
      let url = 'IsoFileSlots?MachineId=' + this.element.getAttribute('machine-id');
    ...
    validateParameters () {
      if (!this.element.hasAttribute('machine-id')) {
        console.log('waiting attribute machine-id in IsoFileSlotBarComponent.element');
        this.setError('missing machine-id'); // delayed error message
        return;
      }
      if (!pulseUtility.isInteger(this.element.getAttribute('machine-id'))) {
        console.error('invalid attribute machine-id in IsoFileSlotBarComponent.element');
      ...
      if (this._range == undefined) {
        console.log('waiting attribute range in IsoFileSlotBarComponent.element');
        if (this.element.hasAttribute('period-context')) {
    ...
    manageSuccess (data) {
      ...
      if ((typeof range !== 'undefined') && (range.isEmpty() || isNaN(range.lower) || isNaN(range.upper))) {
        if (data.Range) {
          console.warn(`x-isofileslotbar:refresh - no begin or end in range - get range=${data.Range} from data`);
          range = pulseRange.createDateRangeFromString(data.Range);
        }
        else {
          console.error('x-isofileslotbar:refresh - no range');
    ...
    refreshRangeData (range, data) {
      ...
      for (let block of data.IsoFileSlots) {
    ...
    draw () {
      ...
      svg.setAttribute('class', 'isofileslotbar-svg');
      ...
      if (this._data) {
        for (let i = 0; i < this._data.length; i++) {
          ...
          //g.setAttribute('range', pulseUtility.convertDateRangeForWebService(range));//range.toString(d => d.toISOString()));
    ...
    displayError (text) {
      ...
      /*if (typeof this.content !== 'undefined') {
        this.content.find('.isofileslotbar-svg').remove();
      }*/
    }
    ...
    onClick (event, range) {
      ...
      pulseDetailsPopup.clickOnBar(this, this._range, applicableRange, event, 'isofile');
    }
  }

  pulseComponent.registerElement('x-isofileslotbar', IsoFileSlotBarComponent, ['machine-id', 'height', 'range', 'period-context', 'machine-context']);
