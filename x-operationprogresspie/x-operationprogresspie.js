...
(function () {
  class OperationProgressPieComponent extends pulseComponent.PulseParamAutoPathRefreshingComponent {
    ...
    constructor(...args) {
      ...
      // proche de animationSeconds = this._animationEndServerDateTimev (= end operation or seq) - 'now'
    ...
    _startDashTimeRefreshTimer (seconds) {
      ...
      console.log('OperationProgressPie(' + this.element.getAttribute('machine-id') + ') '
        + ' Start timer(' + seconds + ')');
      // Re-start timer
      var self = this;
      this._dashTimeRefreshTimer = setTimeout(
        function () {
          /*console.log('OperationProgressPie(' + self.element.getAttribute('machine-id')
            + ' timer called');*/
    ...
    _checkTimeAndAnimateTimeAndDash () {
      ...
      if (!pulseUtility.isNotDefined(this._eventDateTime)) {
        ...
        if (this._activeEvent) {
          if (this._untilNextMSec >= 0) {
            console.error('OperationProgressPie(' + this.element.getAttribute('machine-id') + ') '
      ...
      if (this._needChangeDash == true) {
        ...
        if (!animateTimeAndDash || stopDashAnimation) {
          ...
          // Animate dash circle
          let keyframeName = 'operationprogress-dash-' + this.element.getAttribute('machine-id') + '-' + secondsForDash + 's';
          if (this._activeEvent) {
    ...
    _drawArc (svg, from, to) {
      ...
      if (this._completion != null) {
        ...
        //console.log('OperationProgressPie = ' + completionInSegment);
      }
      else {
        console.log('OperationProgressPie(' + this.element.getAttribute('machine-id')
        ...
        if (beginOfBk < endOfBk) { // To avoid unwanted full ring
          ...
          if (circleBk != null) {
            ...
            if (realEndOfAnimation != null
              ...
              // Animate Bk
              let keyframeNameBk = 'operationprogress-pie-bk-'
        ...
        if (circleProgress != null) {
          ...
          if (animationSeconds > 0) {
            if (realEndOfAnimation != null
              ...
              let keyframeName = 'operationprogress-pie-'
                + this.element.getAttribute('machine-id');
    ...
    _createFixedDashCircle (svg) {
      ...
      let bkColoredCircle = pulseSvg.createCircle(
        this._xyPosition, this._xyPosition, this._dashCircleRadius,
        'transparent', 'operationprogress-dashed',
    ...
    _findTopTextAndStoreEventData () {
      ...
      if (this._data.ActiveEvents && this._data.ActiveEvents.length > 0) {
        ...
        return event.Message; // stop soon / End of operation soon...
      }
      else { // NO EVENT
        //console.log('OperationProgressPie(' + this.element.getAttribute('machine-id')
        //+ '): No event');
    ...
    _restoreDefaultValues () {
      ...
      this._endOfOperationPercent = 1; // Can be overload if all seq < 100%
      this._animationEndPercent = null;
      this._animationEndServerDateTime = null;
    ...
    _draw () {
      ...
      let circleBk = pulseSvg.createSegmentOnDonut(
        this._xyPosition, this._xyPosition, this._circleRadius,
        'transparent', 'operationprogresspie-invisible-bk',
      ...
      if (this._data) {
        ...
        if (pulseUtility.isNotDefined(this._data.Completion)) {
          console.error('OperationProgressPie(' + this.element.getAttribute('machine-id') + '): NO completion = NO animation (probably between operations)');
          this._completion = null;
        }
        else {
          ...
          //console.log('OperationProgressPie(' + this.element.getAttribute('machine-id') + '): completion= ' + this._completion);
          ...
          if ((1 == this._data.ByMachineModule.length)
            ...
            if (!pulseUtility.isNotDefined(lastSeqEndPercent)
              ...
              if (lastSeqEndPercent <= 1) {
                this._endOfOperationPercent = lastSeqEndPercent;

                // Default -> can be over-written below
                this._animationEndPercent = this._endOfOperationPercent;
                this._animationEndServerDateTime = new Date(this._data.EstimatedOperationEndDateTime);

                console.log('OperationProgressPie(' + this.element.getAttribute('machine-id')
                  + '): anim end = end cycl ' + this._animationEndPercent
                  + ' until ' + this._data.EstimatedOperationEndDateTime);
              }
              //else { // Ignore if more than 1 -> an error should be displayed in cog (done in C#)
              //this._endOfOperationPercent = null; // Do not display circle !?!
              //console.warn('OperationProgressPie(' + this.element.getAttribute('machine-id')
            ...
            //this._endOfOperationPercent = 1; // info
            // Default -> can be over-written below
            this._animationEndPercent = this._endOfOperationPercent;
            this._animationEndServerDateTime = new Date(this._data.EstimatedOperationEndDateTime);
                  ...
                  if (seq.EndPercent <= this._animationEndPercent) {
                    if (this._completion < 1) { // else division by 0
                      if (this._animationEndPercent < this._completion) { // Animation positive uniquement
                        ...
                        let operationEndDate = new Date(this._data.EstimatedOperationEndDateTime);
                        this._animationEndServerDateTime = new Date(
                          operationEndDate.getTime()
                          - ((operationEndDate.getTime() - dataDate.getTime()
            ...
            /*this._drawArc(svg, 0, this._endOfOperationPercent);*/
          }
            ...
            console.log('OperationProgressPie(' + this.element.getAttribute('machine-id')
              + '): BLUE IN - NO module ? anim end = end operation = ' + this._animationEndPercent
              + ' until ' + this._animationEndServerDateTime.toISOString());
          ...
          if (previousDrawingEnd < this._endOfOperationPercent) {
            // Draw LAST segment
            //console.log('OperationProgressPie(' + this.element.getAttribute('machine-id')
            //+ '): last call to drawArc');
            this._drawArc(g, previousDrawingEnd, this._endOfOperationPercent);
          }
    ...
    _findAndDisplayErrorsWarnings (svg) {
      if (!pulseUtility.isNotDefined(this._data.Errors) && this._data.Errors.length > 0 && this._data.Errors[0] != '') {
        ...
        errorimg.setAttribute('class', 'operationprogresspie-error-image');
        ...
        let errorimg = this._getCogPath();
        errorimg.setAttribute('class', 'operationprogresspie-warn-image');
    ...
    validateParameters () {
      if (!this.element.hasAttribute('group')) {
        ...
        if (!this.element.hasAttribute('machine-id')) {
          console.error('missing attribute machine-id in OperationProgressPie.element');
      ...
      if (this._animationEndServerDateTime != null) {
        ...
        if (animationMSeconds < (refreshMs - 1000)) {
          if (animationMSeconds > 0) {
            // Next end of operation or sequence is sooner
    ...
    getShortUrl () {
      ...
      //url += '&Light=true'; // If NOT by machine module
    ...
    /**
     * Event bus callback triggered when ...
     *
     * @param {Object} event
     */
  ...
  pulseComponent.registerElement('x-operationprogresspie', OperationProgressPieComponent,
    ['machine-id', 'group', 'textchange-context', 'threshold1', 'threshold2', 'status-context']);
})();
