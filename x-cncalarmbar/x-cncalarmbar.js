...
(function () {
  ...
  class CncAlarmBarComponent extends pulseComponent.PulseParamAutoPathRefreshingComponent {
    ...
    */
     ...
     */
    getShortUrl () {
      let url = 'CncAlarm/Color?MachineId=' + this.element.getAttribute('machine-id');
    ...
    validateParameters () {
      if (!this.element.hasAttribute('machine-id')) {
        console.log('waiting attribute machine-id in CncAlarmBarComponent.element');
        this.setError('missing machine-id'); // delayed error message
        return;
      }
      if (!pulseUtility.isInteger(Number(this.element.getAttribute('machine-id')))) {
        console.error('invalid attribute machine-id in CncAlarmBarComponent.element');
      ...
      if (this._range == undefined) {
        console.log('waiting attribute range in CncAlarmBarComponent.element');
        if (this.element.hasAttribute('period-context')) {
    ...
    manageSuccess (data) {
      ...
      if ((typeof range !== 'undefined') && (range.isEmpty() || isNaN(range.lower) || isNaN(range.upper))) {
        if (data.Range) {
          console.warn(`x-cncalarmbar:refresh - no begin or end in range - get range=${data.Range} from data`);
          range = pulseRange.createDateRangeFromString(data.Range);
        }
        else {
          console.error('x-cncalarmbar:refresh - no range');
    ...
    draw () {
      ...
      svg.setAttribute('class', 'cncalarmbar-svg');
    ...
    displayError (text) {
      ...
      /*if (typeof this.content !== 'undefined') {
        this.content.find('.cncalarmbar-svg').remove();
      }*/
    }
    ...
    /*onClick (event, range) {
      ...
      pulseDetailsPopup.clickOnBar(this, this._range, applicableRange, event, 'cncalarm');
    }*/
  }

  pulseComponent.registerElement('x-cncalarmbar', CncAlarmBarComponent, ['machine-id', 'height', 'range', 'period-context', 'machine-context']);
