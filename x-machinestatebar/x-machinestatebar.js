...
(function () {
  ...
  class MachineStateBarComponent extends pulseComponent.PulseParamAutoPathRefreshingComponent {
    ...
    */
     ...
     */
    getShortUrl () {
      let url = 'MachineStateTemplateSlots?MachineId=' + this.element.getAttribute('machine-id');
    ...
    validateParameters () {
      if (!this.element.hasAttribute('machine-id')) {
        console.log('waiting attribute machine-id in MachineStateBarComponent.element');
        this.setError(this.getTranslation('error.selectMachine', 'Please select a machine')); // delayed error message
        return;
      }
      if (!pulseUtility.isInteger(this.element.getAttribute('machine-id'))) {
        console.error('invalid attribute machine-id in MachineStateBarComponent.element');
      ...
      if (this._range == undefined) {
        console.log('waiting attribute range in MachineStateBarComponent.element');
        if (this.element.hasAttribute('period-context')) {
    ...
    manageSuccess (data) {
      ...
      if ((typeof range !== 'undefined') && (range.isEmpty() || isNaN(range.lower) || isNaN(range.upper))) {
        if (data.Range) {
          console.warn(`x-machinestatebar:refresh - no begin or end in range - get range=${data.Range} from data`);
          range = pulseRange.createDateRangeFromString(data.Range);
        }
        else {
          console.error('x-machinestatebar:refresh - no range');
    ...
    refreshRangeData (range, data) {
      ...
      for (let block of data.MachineStateTemplateSlots) {
    ...
    draw () {
      ...
      svg.setAttribute('class', 'machinestatebar-svg');
      ...
      if (this._data) {
        for (let i = 0; i < this._data.length; i++) {
          ...
          if (!this._isRangeInModifications(range)) {
            ...
            display.setAttribute('font-size', actualHeight * 0.65);
