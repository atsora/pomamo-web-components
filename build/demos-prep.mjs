...
import {
  readdirSync, readFileSync, writeFileSync, appendFileSync, mkdirSync, rmSync, existsSync, statSync, cpSync,
} from 'node:fs'
...
// Landing pages: static files demo/<name>.html (no partials), baked on a full prep only
const LANDINGS = ['index', 'alphabetical']

// Demo links of demo/index.html: [{ page, label, category }], in the index order
function readIndexLinks () {
  const html = readFileSync(resolve(PWC, 'demo/index.html'), 'utf8')
  const links = []
  let category = ''
  for (const m of html.matchAll(/<h3 class="title">(.*?)<\/h3>|<li><a href="([\w-]+)\.html">(.*?)<\/a><\/li>/g)) {
    if (m[1] !== undefined) category = m[1].trim()
    else links.push({ page: m[2], label: m[3].trim(), category })
  }
  return links
}

// tagConfig.displayedPages (left navigation of the demos) = the index: one group
// (title) per category, one entry (subTitle) per link. Appended to config_demo.js.
function displayedPagesScript () {
  const pages = readIndexLinks().map(l => ({ pageName: l.page, title: l.category, subTitle: l.label }))
  return '\n// Generated by build/demos-prep.mjs from demo/index.html\n'
    + `tagConfig.displayedPages = ${JSON.stringify(pages, null, 2)};\n`
}

// <!--(alphabetical-list)--> -> the demos of the index sorted by component name,
// grouped by first letter
function alphabeticalList () {
  const links = readIndexLinks().sort((a, b) => a.page.localeCompare(b.page))
  let out = ''
  let letter = ''
  for (const l of links) {
    const first = l.page.charAt(0).toUpperCase()
    if (first !== letter) {
      if (letter) out += '        </ol>\n      </div>\n'
      letter = first
      out += `      <h3 class="title">${letter}</h3>\n      <div class="view-box">\n        <ol class="list-title">\n`
    }
    out += `          <li><a href="${l.page}.html">x-${l.page}</a> <span class="list-category">${l.category} · ${l.label}</span></li>\n`
  }
  if (letter) out += '        </ol>\n      </div>\n'
  return out
}

// HTML of a landing page, ready to be staged
export function bakeLanding (name) {
  return readFileSync(resolve(PWC, `demo/${name}.html`), 'utf8')
    .replaceAll('{{pagename}}', name)
    .replace('<!--(alphabetical-list)-->', () => alphabeticalList())
}
...
export function prepDemos (filter = null) {
  ...
  // landing pages (their own static files, no partials) — only on a full prep
  const landings = filter ? [] : LANDINGS.filter(n => existsSync(resolve(PWC, `demo/${n}.html`)))
  for (const n of landings) writeFileSync(resolve(stagingPages, `${n}.html`), bakeLanding(n))
  ...
  appendFileSync(resolve(scriptsOut, 'config_demo.js'), displayedPagesScript())
  ...
  // shared by all the landing pages
  if (landings.length) lessc(resolve(PWC, 'demo/demo.less'), resolve(stylesOut, 'index.css'))
