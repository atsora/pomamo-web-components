...
/**
 * @module x-detailedproductionstateat
...
(function () {
  class DetailedProductionStateAtComponent extends pulseComponent.PulseParamAutoPathSingleRequestComponent {
    ...
    initialize () {
      ...
      let title = this.getTranslation(
        'detailsViewSubTitles.productionstate', 'production state');
      let spanTitle = $('<span></span>').addClass('detailedproductionstateat-title-span')
        .html(title);
    ...
    validateParameters () {
      if ((!this.element.hasAttribute('machine-id'))
        ...
        console.error('missing attribute machine-id in detailedproductionstateat.element');
        // Delayed display :
      ...
      if (!this.element.hasAttribute('when')) {
        console.error('missing attribute when in detailedproductionstateat.element');
        // Delayed display :
    ...
    getShortUrl () {
      ...
      return 'ProductionStateSlots?GroupId='
    ...
    refresh (data) {
      ...
      if (0 < data.ProductionStateSlots.length) {
        let divRange = $('<div></div>').addClass('detailed-range');
        let divDetails = $('<div></div>').addClass('detailed-data');

        // RANGE
        let tmpRange = pulseRange.createDateRangeFromString(data.ProductionStateSlots[0].Range);
        let rangeDisplay = pulseUtility.displayDateRange(tmpRange, true);
        let spanRange = $('<span></span>').addClass('detailedproductionstateat-range-span')
          .html(rangeDisplay);
        $(divRange).append(spanRange);

        // DETAILS
        let display = data.ProductionStateSlots[0].Display;
  ...
  pulseComponent.registerElement('x-detailedproductionstateat', DetailedProductionStateAtComponent, ['machine-id', 'when', 'datetime-context', 'machine-context']);
})();
