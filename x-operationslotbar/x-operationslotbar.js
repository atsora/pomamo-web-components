...
(function () {
  ...
  /**
   * `<x-operationslotbar>` — timeline bar showing operation slots
   * (work order / component / operation) for one machine over a date range.
   *
   * Polls `OperationSlots?MachineId=<id>&Range=<range>` and renders one
   * colored SVG segment per slot proportional to the range. Clicks open
   * the operation details view through `pulseDetailsPopup` (`showdetails` /
   ...
   */
  class OperationSlotBarComponent extends pulseComponent.PulseParamAutoPathRefreshingComponent {
    /**
     * Constructor
     *
     * @param  {...any} args
    ...
    */
     ...
     */
    getShortUrl () {
      let url = 'OperationSlots?MachineId=' + this.element.getAttribute('machine-id');
    ...
    initialize () {
      ...
      this._content.className = 'operationslotbar-content pulse-bar-content';
      ...
      this.element.classList.add('operationslotbar');
    ...
    validateParameters () {
      if (!this.element.hasAttribute('machine-id')) {
        console.log('waiting attribute machine-id in OperationSlotBarComponent.element');
        this.setError(this.getTranslation('error.selectMachine', 'Please select a machine')); // delayed error message
        return;
      }
      if (!pulseUtility.isInteger(this.element.getAttribute('machine-id'))) {
        console.error('invalid attribute machine-id in OperationSlotBarComponent.element');
      ...
      if (this._range == undefined) {
        console.log('waiting attribute range in OperationSlotBarComponent.element');
        if (this.element.hasAttribute('period-context')) {
    ...
    manageSuccess (data) {
      ...
      if ((typeof range !== 'undefined') && (range.isEmpty() || isNaN(range.lower) || isNaN(range.upper))) {
        if (data.Range) {
          console.warn(`x-operationslotbar:refresh - no begin or end in range - get range=${data.Range} from data`);
          range = pulseRange.createDateRangeFromString(data.Range);
        }
        else {
          console.error('x-operationslotbar:refresh - no range');
    ...
    cleanContent () {
      ...
      let oldSvg = this.element.querySelector('.operationslotbar-svg');
      if (oldSvg) {
        oldSvg.remove();
      }
    }
    ...
    displayError (text) {
      ...
      /*if (typeof this.content !== 'undefined') {
        this.content.querySelector('.operationslotbar-svg').remove();
      }*/
    }
  ...
  pulseComponent.registerElement('x-operationslotbar', OperationSlotBarComponent, ['machine-id', 'height', 'range', 'period-context', 'machine-context']);
})();
