...
(function () {
  ...
  class ShiftSlotBarComponent extends pulseComponent.PulseParamAutoPathRefreshingComponent {
    ...
    */
     ...
     */
    get refreshRate () {
      if (this._range.upper < new Date()) { // Past period completed
    ...
    getShortUrl () {
      ...
      // TO USE ASAP : url += '&Range=' + pulseUtility.convertDateRangeForWebService(this._range);
      /* To add once done :
    ...
    initialize () {
      ...
      this._content.className = 'shiftslotbar-content pulse-bar-content';
      ...
      this.element.className = 'shiftslotbar';
    ...
    validateParameters () {
      ...
      if (this._range == undefined) {
        console.log('waiting attribute range in ShiftSlotBarComponent.element');
        if (this.element.hasAttribute('period-context')) {
     ...
     */
    manageSuccess (data) {
      // data.Begin && data.End exists here
      ...
      if ((typeof range !== 'undefined') && (range.isEmpty() || isNaN(range.lower) || isNaN(range.upper))) {
        if (data.Range) {
          console.warn(`x-shiftslotbar:refresh - no begin or end in range - get range=${data.Range} from data`);
          range = pulseRange.createDateRangeFromString(data.Range);
        }
        else {
          console.error('x-shiftslotbar:refresh - no range');
    ...
    refreshRangeData (range, data) {
      ...
      for (let block of data.List) {
        //let blockRange = pulseRange.createDateRangeFromString(block.Range);
        let blockRange = pulseRange.createDefaultInclusivity((block.ShiftBegin == '') ? barbegin : new Date(block.ShiftBegin), (block.ShiftEnd == '') ? barend : new Date(block.ShiftEnd));
        ...
        //slot.isobegin = pulseUtility.convertMomentToDateTimeString(moment(iBegin));
        //slot.isoend = pulseUtility.convertMomentToDateTimeString(moment(iEnd));
    ...
    cleanContent () {
      ...
      let svgs = this.element.querySelectorAll('.shiftslotbar-svg');
      svgs.forEach(el => el.remove()); // Remove Old SVG
    }
    ...
    draw () {
      ...
      // Use the actual rendered height (CSS may override the height via flex layout)
      ...
      svg.setAttribute('class', 'shiftslotbar-svg');
      ...
      if (this._data) {
        for (let i = 0; i < this._data.length; i++) {
          ...
          if (undefined === this._data[i].bgColor)
            rect.setAttribute('fill', 'tranparent');
          else
          ...
          if (undefined !== this._data[i].display) {
            // Add Text
            ...
            if (pulseUtility.isNotDefined(this._data[i].fgColor)) { // Always
              if (undefined === this._data[i].bgColor)
                display.setAttribute('fill', 'black');
              else
                display.setAttribute('fill', pulseUtility.getContrastColor(this._data[i].bgColor));
            }
            else {
              display.setAttribute('fill', this._data[i].fgColor); // Later -- 2017-04
            }
    ...
    displayError (text) {
      ...
      /*if (typeof this.content !== 'undefined') {
        let svgs = this.content.querySelectorAll('.shiftslotbar-svg');
    ...
    onClick (event, range) {
      ...
      pulseDetailsPopup.clickOnBar(this, this._range, applicableRange, event, 'shift');
    }
  }

  pulseComponent.registerElement('x-shiftslotbar', ShiftSlotBarComponent, ['height', 'range', 'period-context']);
})();
