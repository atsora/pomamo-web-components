...
(function () {
  ...
  /**
   * `<x-observationstatebar>` — timeline bar showing observation-state slots
   * for one machine over a date range.
   *
   * Polls `ObservationStateSlots?MachineId=<id>&Range=<range>` and renders
   * one colored SVG segment per slot proportional to the range. Clicks open
   * the details view through `pulseDetailsPopup` (`showdetails` / `showpopup`
   * configs select an in-place details page or a floating popup). Pending
   * modifications matching the current machine append an
   * `x-revisionprogress` over the affected sub-range and trigger a reload
   * once their `pendingModifications === 0`. Reacts to
  ...
  class ObservationStateBarComponent extends pulseComponent.PulseParamAutoPathRefreshingComponent {
    ...
    */
     ...
     */
    getShortUrl () {
      let url = 'ObservationStateSlots?MachineId=' + this.element.getAttribute('machine-id');
    ...
    initialize () {
      ...
      this._content.className = 'observationstatebar-content pulse-bar-content';
      ...
      this.element.classList.add('observationstatebar');
    ...
    validateParameters () {
      ...
      if (this._range == undefined) {
        console.log('waiting attribute range in ObservationStateBarComponent.element');
        if (this.element.hasAttribute('period-context')) {
    ...
    manageSuccess (data) {
      ...
      if ((typeof range !== 'undefined') && (range.isEmpty() || isNaN(range.lower) || isNaN(range.upper))) {
        if (data.Range) {
          console.warn(`x-observationstatebar:refresh - no begin or end in range - get range=${data.Range} from data`);
          range = pulseRange.createDateRangeFromString(data.Range);
        }
        else {
          console.error('x-observationstatebar:refresh - no range');
    ...
    refreshRangeData (range, data) {
      ...
      for (let block of data.ObservationStateSlots) {
        ...
        //slot.patternName = block.PatternName; Not used anymore
        slot.patternColor = block.PatternColor;
    ...
    cleanContent () {
      ...
      let oldSvg = this.element.querySelector('.observationstatebar-svg');
      if (oldSvg) {
        oldSvg.remove();
      }
    }
    ...
    draw () {
      ...
      // Use the actual rendered height (CSS may override the vanilla DOM-set height via flex layout)
      ...
      if (this._data) {
        for (let i = 0; i < this._data.length; i++) {
          ...
          if (!this._isRangeInModifications(range)) {
            ...
            g.setAttribute('height', topBarHeight);
            ...
            rect.setAttribute('height', topBarHeight);
            ...
            display.setAttribute('font-size', topBarHeight / 2 + 1); // + 1 to make font bigger
            ...
            if (!pulseUtility.isNotDefined(this._data[i].patternColor)) {
              // Add "Pattern" -> bottom bar
              let bottomRect = document.createElementNS(pulseSvg.get_svgNS(), 'rect');
              bottomRect.setAttribute('x', x);
              bottomRect.setAttribute('y', topBarHeight);
              bottomRect.setAttribute('width', width);
              bottomRect.setAttribute('height', bottomBarHeight);
              bottomRect.setAttribute('fill', this._data[i].patternColor);
              g.appendChild(bottomRect);
            }
    ...
    displayError (text) {
      ...
      /*if (typeof this.content !== 'undefined') {
        let oldSvg = this.content.querySelector('.observationstatebar-svg');
    ...
    onClick (event, range) {
      ...
      pulseDetailsPopup.clickOnBar(this, this._range, applicableRange, event, 'observationstate');
    }
  }

  pulseComponent.registerElement('x-observationstatebar', ObservationStateBarComponent, ['machine-id', 'height', 'range', 'period-context', 'machine-context']);
})();
